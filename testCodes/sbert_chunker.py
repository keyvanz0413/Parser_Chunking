...
class ChunkingConfig:
    ...
    # Sentence-BERT model for semantic analysis
    SBERT_MODEL_NAME = "all-MiniLM-L6-v2"    # Lightweight but effective model
    ENABLE_SEMANTIC_ANALYSIS = True          # Toggle semantic layer
    
    # Semantic chunking settings
    SEMANTIC_SIMILARITY_THRESHOLD = 0.5      # Similarity drop below this triggers split
    SEMANTIC_WINDOW_SIZE = 3                 # Sentences to consider for moving average
    ENABLE_SEMANTIC_CHUNKING = True          # Use semantic breakpoints for chunking
    
    # Role prototype matching settings
    ROLE_PROTOTYPE_SIMILARITY_THRESHOLD = 0.75  # Min similarity to assign role via semantic
    ENABLE_PROTOTYPE_MATCHING = True          # Use prototype matching for role refinement
...
class EnrichedChunk:
    ...
    v2.0 Semantic Analysis Fields:
    - role_distribution: Dict mapping role -> count (e.g., {"definition": 2, "example": 1})
    - primary_logic_path: Sequential role flow (e.g., "topic -> definition -> example")
    - semantic_coherence: Float 0-1 indicating internal semantic consistency
    - split_reason: Why this chunk was split here (semantic_drop, role_change, size_limit)
    """
    chunk_id: str
    heading_path: str
    chunk_type: str
    ...
    split_reason: str = "none"  # semantic_drop, role_change, size_limit, none
    


# =============================================================================
# Semantic Analyzer (v2.0 Upgrade - SBERT-based)
# =============================================================================
...
class SemanticAnalyzer:
    ...
    3. Role prototype matching (refine 'explanation' to specific roles)
    4. Semantic breakpoint detection (topic drift detection)
    5. Cross-page semantic continuity scoring
    
    This class is designed to augment rule-based analysis, not replace it.
    When rules are confident, semantics are used for validation.
    When rules are uncertain (return 'explanation'), semantics provide refinement.
    """
    
    # Role prototypes: Example sentences representing each role
    # These will be embedded once during initialization
    ROLE_PROTOTYPES = {
        'definition': [
    ...
    def __init__(self, config: ChunkingConfig = None):
        ...
        self._model = None
        self._role_embeddings = {}  # {role: np.array of mean embedding}
    ...
    def _lazy_init(self):
        ...
        if not self.config.ENABLE_SEMANTIC_ANALYSIS:
            logger.info("Semantic analysis disabled in config")
            return False
        ...
        try:
            ...
            logger.info(f"Loading SBERT model: {self.config.SBERT_MODEL_NAME}")
            self._model = SentenceTransformer(self.config.SBERT_MODEL_NAME)
            self._np = np
            
            # Pre-compute role prototype embeddings
            if self.config.ENABLE_PROTOTYPE_MATCHING:
                self._compute_role_embeddings()
            
            self._initialized = True
            logger.info("SemanticAnalyzer initialized successfully")
            return True
            
        except ImportError as e:
            logger.warning(
                f"sentence-transformers not installed. Semantic analysis disabled. "
    ...
    def encode_sentences(self, sentences: List[str]) -> Optional[Any]:
        ...
        Returns:
            numpy array of shape (n_sentences, embedding_dim) or None if not initialized
        """
    ...
    def find_semantic_breakpoints(self, sentences: List[str]) -> List[Dict[str, Any]]:
        """
        Detect semantic breakpoints in a sequence of sentences.
        
        A breakpoint occurs when the semantic similarity between consecutive
        sentences drops significantly below the moving average.
        
        Args:
            sentences: List of sentence strings
            
        Returns:
            List of dicts with breakpoint info:
            [{"index": int, "similarity": float, "avg_similarity": float, "drop_ratio": float}]
        """
        if not self._lazy_init() or len(sentences) < 2:
            return []
        ...
        # Find breakpoints using moving average
        breakpoints = []
        window_size = min(self.config.SEMANTIC_WINDOW_SIZE, len(similarities))
        threshold = self.config.SEMANTIC_SIMILARITY_THRESHOLD
        
        for i in range(len(similarities)):
            # Compute moving average of previous similarities
            ...
            # Breakpoint if: current << average AND current < absolute threshold
            drop_ratio = current_sim / avg_sim if avg_sim > 0 else 1.0
            
            if current_sim < threshold and drop_ratio < 0.7:
                breakpoints.append({
                    "index": i + 1,  # Breakpoint is AFTER sentence i
    ...
    def refine_role_by_semantics(self, sentence: str, current_role: str) -> Tuple[str, float]:
        ...
        # Only refine if current role is uncertain
        if current_role not in ('explanation', 'irrelevant'):
            return current_role, 1.0
    ...
    def compute_chunk_coherence(self, sentences: List[str]) -> float:
        ...
        High coherence = sentences are semantically related (good chunk).
        Low coherence = sentences cover disparate topics (may need splitting).
    ...
    def compute_cross_page_semantic_score(self, prev_text: str, curr_text: str) -> float:
        """
        Compute semantic continuity score between end of previous page
        and start of current page.
        
        Used to augment bbox-based cross-page detection.
        
        Returns:
            Float similarity score between 0 and 1
        """
        if not self._lazy_init():
            return 0.5  # Neutral score if not initialized
            
        # Get last sentence of prev, first sentence of curr
        # Simplified: use last 100 chars of prev and first 100 chars of curr
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Return semantic analyzer statistics."""
        return {
            "initialized": self._initialized,
            "model_name": self.config.SBERT_MODEL_NAME if self._initialized else None,
            "roles_with_prototypes": list(self._role_embeddings.keys()),
            "prototype_count": sum(len(v) for v in self.ROLE_PROTOTYPES.values()),
        }
...
class POSAnalyzer:
    ...
    v2.0 Enhancements:
    - SemanticAnalyzer integration for role refinement
    - Semantic similarity-based role correction for uncertain cases
    - Role distribution tracking for chunk metadata
    """
    ...
    def __init__(self, model_name: str = "en_core_web_md", config: ChunkingConfig = None):
        ...
        # v2.0: Initialize SemanticAnalyzer for role refinement
    ...
    def analyze_sentences(self, text: str, heading_path: str = "") -> List[Dict[str, Any]]:
        ...
        for i, sent in enumerate(sent_list):
            ...
            original_role = role
            if self._semantic_analyzer and role in ('explanation', 'irrelevant'):
                refined_role, semantic_confidence = self._semantic_analyzer.refine_role_by_semantics(
                    sent_text, role
                )
                if refined_role != role:
                    logger.debug(f"Semantic refinement: '{role}' -> '{refined_role}' (conf: {semantic_confidence:.2f})")
                    role = refined_role
            ...
            sentences.append({
                ...
                # v2.0: Additional semantic metadata
                "original_role": original_role if original_role != role else None,
                "semantic_confidence": round(semantic_confidence, 3) if semantic_confidence > 0 else None,
            })
    ...
    def _determine_role(self, text: str, position: int, pos_tags: List[str], 
        ...
        # ============ QUESTION (interrogative form) - Highest priority ============
        if form == "interrogative" or text_stripped.endswith('?'):
            return "question"  # Early return, very confident
        
        # ============ IRRELEVANT (with exceptions) ============
        # v2.1: Be more careful - short doesn't always mean irrelevant
        if len(text_stripped) < 10:
            has_exception = any(p.search(text) for p in self._short_exceptions)
            if not has_exception:
                candidate_role = "irrelevant"
        
        # Boilerplate / Navigation / URLs
        if re.search(r"^\s*(see\s+(?:also|below|above)|page\s+\d+|continued|ibid)\s*$", text_lower):
            return "irrelevant"
        if re.search(r"\b(?:http|www\.|\.com|\.org)\b", text_lower):
            return "irrelevant"
        if re.search(r"\b(?:all\s+rights\s+reserved|copyright)\b", text_lower):
            return "irrelevant"
        
        # ============ TOPIC DETECTION (v2.2: Conservative approach) ============
        # Topic sentences are difficult to detect reliably - be conservative
        # Only mark as topic for clearly identifiable patterns
        is_topic_candidate = False
        # Only apply topic detection to first sentence with specific patterns
        if candidate_role == "explanation" and is_first:
            word_count_check = len(text_stripped.split())
            if word_count_check < 15 and word_count_check >= 4:
                # Must start with capital and have a verb
                if "VERB" in pos_tags:
                    # Exclude subordinating conjunctions
        ...
        if candidate_role == "explanation":
            if re.search(r"\b(assume|assuming|given\s+that|suppose|provided\s+that|let\s+\w+\s+(?:be|equal|denote))\b", text_lower):
                candidate_role = "assumption"
            elif re.search(r"^(?:Suppose|Assume|Given|Let)\b", text):
                candidate_role = "assumption"

        # ============ CONCLUSION ============
        if candidate_role == "explanation":
            if re.search(r"\b(therefore|thus|hence|in\s+conclusion|as\s+a\s+result|consequently|to\s+summarize|in\s+summary|the\s+takeaway)\b", text_lower):
                candidate_role = "conclusion"
            elif is_last and re.search(r"\b(overall|ultimately|finally)\b", text_lower):
                candidate_role = "conclusion"

        # ============ COMPARISON ============
        if candidate_role == "explanation":
            if re.search(r"\b(compared?\s+to|in\s+contrast|unlike|whereas|similarly|on\s+the\s+other\s+hand)\b", text_lower):
                candidate_role = "comparison"
            elif re.search(r"^(?:Unlike|Whereas|Similarly|In\s+contrast|Compared)\b", text):
                candidate_role = "comparison"
            elif re.search(r"\b(more|less|greater|smaller|higher|lower|better|worse)\s+than\b", text_lower):
                candidate_role = "comparison"
        ...
        # ============ PROCEDURE (v2.1: Tighter rules) ============
        # Only classify as procedure if there are strong procedural signals
        if candidate_role == "explanation":
            # Strong signals: step numbers, explicit methodology
            if re.search(r"^(?:Step\s+\d|First,|Second,|Third,|Finally,|Next,|Then,)\s", text):
                candidate_role = "procedure"
            elif re.search(r"\b(?:step\s+\d+|algorithm|methodology|follow\s+these\s+steps)\b", text_lower):
                candidate_role = "procedure"
            # v2.1: Only use imperative if it's clearly instructional (longer text)
            elif is_imperative and word_count >= 8:
                # Exclude short imperative topic sentences like "Hold this stock"
                if re.search(r"\b(calculate|compute|determine|solve|find|use|apply|enter|click|select)\b", text_lower):
                    candidate_role = "procedure"

        # ============ DEFINITION ============
        if candidate_role == "explanation":
            if re.search(r"\bis\s+(?:defined\s+as|the\s+\w+\s+(?:of|that))\b", text_lower):
                candidate_role = "definition"
            elif re.search(r"\brefers?\s+to\b|\bmeans?\s+that\b", text_lower):
                candidate_role = "definition"
            elif re.search(r"^[A-Z][a-z]+\s+is\s+a\s+(?:measure|type|form|kind|method|process)\b", text):
                candidate_role = "definition"

        # ============ FORMULA ============
        if candidate_role == "explanation" or candidate_role == "evidence":
            # v2.1: Tighter formula detection
            if re.search(r"[A-Z]\s*=\s*[A-Z\d(]", text):  # Variable = expression
                candidate_role = "formula"
            elif re.search(r"\bequation\b|\bformula\b", text_lower):
                candidate_role = "formula"
            elif re.search(r"[=+*/^]{2,}", text):  # Multiple operators
                candidate_role = "formula"

        # ============ INTERPRETATION ============
        if candidate_role == "explanation":
            if re.search(r"\b(this\s+(?:means|implies|suggests|indicates)|in\s+other\s+words|effectively|the\s+implication)\b", text_lower):
                candidate_role = "interpretation"

        # ============ MECHANISM ============
        if candidate_role == "explanation":
            if re.search(r"\b(mechanism|works?\s+by|functions?\s+by|how\s+\w+\s+works?)\b", text_lower):
                candidate_role = "mechanism"

        # ============ LIMITATION ============
        if candidate_role == "explanation":
            if re.search(r"\b(limitation|constraint|caveat|does\s+not\s+(?:apply|work)|cannot\s+be\s+used|only\s+(?:works?|applies?)\s+(?:when|if|for))\b", text_lower):
                candidate_role = "limitation"
        
        # ============ APPLICATION ============
        if candidate_role == "explanation":
            if re.search(r"\b(in\s+practice|applies?\s+to|used\s+(?:in|for|by)|practical|real-world|practitioners?)\b", text_lower):
                candidate_role = "application"
        
        # ============ EXAMPLE ============
        if candidate_role == "explanation":
            if re.search(r"\b(for\s+example|for\s+instance|such\s+as|e\.g\.|consider\s+(?:a|the|this))\b", text_lower):
                candidate_role = "example"
            elif re.search(r"^(?:Consider|To\s+illustrate|As\s+an\s+example)\b", text):
                candidate_role = "example"
        
        # ============ APPLY TOPIC IF STILL EXPLANATION ============
        # v2.1: Apply topic detection after all other specific roles are checked
        if candidate_role == "explanation" and is_topic_candidate:
            candidate_role = "topic"
        
        # ============ CONTEXTUAL REFINEMENT ============
        # If current is vague (explanation) and follows a strong trigger role
        if candidate_role == "explanation" and prev_role in self.EXPECTED_SEQUENCES:
            # After example, imperative might be procedure
            if prev_role == "example" and is_imperative and word_count >= 10:
                candidate_role = "procedure"
...
class LogicSegmenter:
    ...
    v2.0 Enhancements:
    - SemanticAnalyzer for semantic breakpoint detection
    - Role-driven chunk boundaries (topic/question starts new chunk)
    - Semantic coherence scoring for chunks
    - Role distribution and logic path tracking
    """
    ...
    def __init__(self, use_pos: bool = True, config: ChunkingConfig = None):
        ...
        # v2.0: Initialize SemanticAnalyzer for semantic chunking
        self._semantic_analyzer = None
        if self.config.ENABLE_SEMANTIC_ANALYSIS:
            self._semantic_analyzer = SemanticAnalyzer(self.config)
    ...
    def process_file(self, input_json_path: str, output_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        # v2.0: Semantic analysis features
        if self.config.ENABLE_SEMANTIC_ANALYSIS:
            features.append("semantic_analysis")
        if self.config.ENABLE_SEMANTIC_CHUNKING:
            features.append("semantic_chunking")
        if self.config.ENABLE_PROTOTYPE_MATCHING:
    ...
    def _create_chunk(self, segments: List[Dict], heading_path: str, 
        ...
        # v2.0: Compute semantic metadata
        role_distribution = {}
        primary_logic_path = ""
        ...
        if sentences:
            ...
            for sent in sentences:
                role = sent.get('role', 'explanation')
                role_distribution[role] = role_distribution.get(role, 0) + 1
            
            # Build logic path (sequence of unique roles)
            seen_roles = []
            for sent in sentences:
                role = sent.get('role', 'explanation')
                if not seen_roles or seen_roles[-1] != role:
                    seen_roles.append(role)
            primary_logic_path = " -> ".join(seen_roles[:5])  # Limit to 5 for brevity
            
            # Compute semantic coherence if analyzer available
            if self._semantic_analyzer and len(sentences) >= 2:
                sent_texts = [s.get('text', '') for s in sentences if s.get('text')]
                semantic_coherence = self._semantic_analyzer.compute_chunk_coherence(sent_texts)
        ...
        return EnrichedChunk(
            ...
            # v2.0: Semantic metadata
            role_distribution=role_distribution,
            primary_logic_path=primary_logic_path,
